...
public static class NavmeshIPC
{
    ...
    /// <summary>
    /// Navmesh build progress: &lt; 0 when idle/done, or 0..1 while the zone's mesh is building.
    /// </summary>
    public static float BuildProgress()
        => Invoke<float>("vnavmesh.Nav.BuildProgress");

    /// <summary>
    /// True only when the current zone's navmesh is fully built and ready to path on
    /// (guard movement requests with this so we don't path before the mesh exists).
    /// </summary>
    public static bool MeshReady()
    {
        ...
        // < 0 => no active build (done/idle); [0,1) => still building.
