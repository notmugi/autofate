...
public sealed partial class MainWindow
{
    ...
    private void DrawChocoboTab()
    {
        ...
        if (C.ChocoboLevelingEnabled)
        {
            ...
            ImGui.Spacing();
            ImGui.TextWrapped("Target your chocobo stable in-game, then click below to capture it.");
            if (ImGui.Button("Add targeted stable"))
            {
                var tgt = Svc.Targets.Target;
                if (tgt == null)
                {
                    Svc.Chat.PrintError("[Autofate] No target. Click your chocobo stable first, then press this.");
                }
                else
                {
                    ...
                    Save();
                    Svc.Chat.Print($"[Autofate] Stable added: '{C.StableName}' (DataId {C.StableDataId}).");
                }
            }
